...
* Browser (client): loads [Beefree SDK](https://docs.beefree.io/beefree-sdk) via npm package, initializes it with the server‑provided token, and mounts the editor in a container div.
...
* The Beefree SDK editor loads in the container and then loads the sample template.
...
The Vite dev server will proxy API requests to Django and provide hot reloading for frontend changes.
...
* Django securely generates a Beefree SDK token server‑side via `https://auth.getbee.io/loginV2` using `BEE_CLIENT_ID` and `BEE_CLIENT_SECRET`.
...
* `templates/editor/index.html`: page structure, docs button, editor container div (`id="beefree-editor-container"`), and a `<script type="module">` inclusion for the built JavaScript.
...
* `package.json`: Defines npm dependencies including `@beefree.io/sdk`.
* `vite.config.js`: Configures the build process to output to Django's static directory.
* `npm run build`: Bundles the frontend code and assets for production.

**Template initialization**

* The starter template is embedded directly in `frontend/src/editor.js` as the `initialTemplate` constant.
* Update the template object to change the default content.
